...
class Colourizer(object):
    ...
    def _low_network(self, x):
        ...
        conv2 = self._conv_layer(conv1,
            name='low_conv2', ksize=3, stride=1, num_in=32, num_out=64) # lateral32
        ...
        conv4 = self._conv_layer(conv3,
            name='low_conv4', ksize=3, stride=1, num_in=64, num_out=128) # lateral16

        return conv4, conv2, conv4
    ...
    def _mid_network(self, x):
        ...
        conv2 = self._conv_layer(conv1,
            name='mid_conv2', ksize=3, stride=1, num_in=128, num_out=64) # lateral8
    ...
    def _colorization_network(self, x, lateral8, lateral16, lateral32):

        x_lateral8 = tf.concat([lateral8, x], axis=3)
        # (b, 8, 8, 64+64 -> (b, 8, 8, 64)
        conv0 = self._conv_layer(x_lateral8,
            name='colour_conv0', ksize=3, stride=1, num_in=64+64, num_out=64) # lateral (+64 filters)

        # First upsample: using nearest neighbours interpolation
        # (b, 8, 8, 64) --> (b, 16, 16, 64)
        upsample1 = tf.image.resize_images(conv0, size=[16, 16],
            method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)

        upsample1_lateral16 = tf.concat([lateral16, upsample1], axis=3)
        # (b, 16, 16, 64+128) --> (b, 16, 16, 64)
        conv1 = self._conv_layer(upsample1_lateral16,
            name='colour_conv1', ksize=3, stride=1, num_in=64+128, num_out=64) # lateral (+128 filters)

        # (b, 16, 16, 64) --> (b, 16, 16, 64)
        conv2 = self._conv_layer(conv1,
            name='colour_conv2', ksize=3, stride=1, num_in=64, num_out=64)

        # Second upsample: using nearest neighbours interpolation
        # (b, 16, 16, 64) --> (b, 32, 32, 64)
        upsample2 = tf.image.resize_images(conv2, size=[32, 32],
            method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)

        upsample2_lateral32 = tf.concat([lateral32, upsample2], axis=3)
        # Third conv layer. 3x3 filter with stride 1. 16 feature maps
        # Output shape: (b, 32, 32, 8)
        conv3 = self._conv_layer(upsample2_lateral32,
            name='colour_conv3', ksize=3, stride=1, num_in=64+64, num_out=32) # lateral (+64 filters)

        # Fourth conv layer. 3x3 filter with stride 1. 16 feature maps
        # Output shape: (b, 32, 32, 2)
        output = self._conv_layer(conv3,
            name='colour_output', ksize=1, stride=1, num_in=32, num_out=2,
            activation=tf.sigmoid, use_bn_scaling=True)
    ...
    def _core_builder(self):
        ...
        with tf.variable_scope("core", reuse=tf.AUTO_REUSE):
            low_features, lateral32, lateral16 = self._low_network(self.l_)
            mid_features, lateral8 = self._mid_network(low_features)
            ...
            ab_predictions = self._colorization_network(
                fused_features, lateral8, lateral16, lateral32)
